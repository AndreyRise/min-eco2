...
import AdminAddLink from "./AdminAddLink.js";
import AdminLinkInfo from "./AdminLinkInfo.js";


function AllLinksComp(params) {
...
function closeModal() {
  const setClose = () => {setIsOpen(false);setIsOpen2(false)}
        ...
        <div className="relative w-full">
          ...
          <table className="table w-full table-auto border-separate border-spacing-y-[10px] border-black pb-[28px]">
            ...
            <tbody className="max-h-full overflow-y-scroll">
              {links.map((item)=>{
                return (<tr key={item.id} className='bg-slate-50 select-none cursor-pointer hover:scale-[102%] transition-all' onClick={()=>{setPopUpData(item);openModal2()}}>
                    ...
                    <td className="tableContentText border-t-stone-400 border-solid rounded-r-xl">{item.link}</td>
                  </tr>)
              })
              }
            </tbody></>}
          ...
          {isOpen2?
            <AdminLinkInfo isOpen={isOpen2} closeModal={closeModal} openModal={openModal2} popUpData={popUpData}/>:
            <></>
          }
          {isOpen?
            <AdminAddLink isOpen={isOpen} closeModal={closeModal} openModal={openModal}/>:
            <></>
          }
          {!links?<>
...
export default observer(AllLinksComp);